...
class NaverStock:
    def __init__(self):
        self.p_url_kospi = "http://finance.naver.com/sise/sise_market_sum.nhn?sosok=0&page={0}"  # 코스피 주식 종목 정보
        self.p_url_kosdaq = "http://finance.naver.com/sise/sise_market_sum.nhn?sosok=1&page={0}"  # 코스닥 주식 종목 정보
        self.p_url_stockinfo = "http://finance.naver.com/item/frgn.nhn?page={0}&code={1}"  # 주식 상세 가격정보

    #주식코드 정보와 주식명 Dict를 만들어 리턴한다. (p_type = kospi or kosdaq)
    #m_dic_stock[주식코드] = 주식명
    def get_stock_list(self, p_type):

        m_url = ''
        if p_type == "kospi":
            m_url = self.p_url_kospi
        else:
            m_url = self.p_url_kosdaq

        m_page = 1
        m_dic_stock = {}
        while m_page <= 100:
            m_response = requests.get(m_url.format(m_page))
            time.sleep(0.3) # 너무 빠르게 요청하니 네이버에서 차단됨.
    ...
    def get_stock_detail_list(self, p_code, p_early_stop_date):
        ...
        m_page = 1
        while m_page <= 300:
            m_url = self.p_url_stockinfo.format(m_page, p_code)
